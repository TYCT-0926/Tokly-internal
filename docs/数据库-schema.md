...
--     mod 64；片内取位**逐下标独立雪崩**，不用双散列等差列。
--   （2026-08-05 四轮定案：原双散列 h1+j*h2 使一个身份的 k 个下标构成
--    等差数列而彼此相关，实测 FPR 约 1/14,500，是标称 2^-15 的 2.2 倍；
--    「片宽取 2 的幂」的修法经实测反而劣化至 1/2,614——旧的 1480 位宽
--    是被 u64 回绕意外充当混合器救回来的。真实根因是下标推导而非片宽
--    几何。逐下标雪崩后实测 1/36,364；2026-08-06 跨模型复审独立抽样
--    200 万次得 46 个假阳性 ≈ 1/43,478，两次测量均落在上界内。）
