...
## 评审记录（双镜头，Sonnet，与实现者不同上下文）

按验证分层本批属常规档（不触高危清单），执行 契合性核查 + 对抗红队 两路并行，全部结论已裁决落地：

**契合性核查（结论 NEEDS-WORK → 已修）**：核心架构（节流、版本门控、单事务复合查询、密度映射与分位数、降级链、key-form 文案、整列丢弃、主题保位）全部验证通过，含 counted-only 过滤、reasoning 不进总量、票据字节流三档断言。三条发现：

| # | 发现 | 处置 |
|---|---|---|
| 1 | 布局宽度用 `chars().count()`——CJK 名称（2 列字形）会剪切列对齐与状态栏宽度计算；zh-CN 是首发 locale | **已修**（720249c）：全部布局计量改显示单元格（unicode-width，ratatui 既有依赖零新增）；截断按字形边界；CJK 测试钉死 |
| 2 | 键位提示键字用 fg.muted，§13 字面为整段 fg.faint | **已修**（720249c）：整段 faint——规格锁死色值，不即兴分层 |
| 3 | compact() 超 ~999.5T 超出 3 有效数字（T 为末位单位） | **判定为文档化边界**：不发明规则表以外的单位；共享向量新增 `999999999999999 → "1000T"` 钉死该行为（ca344c7） |

**对抗红队（两条实证 + 两条机理，全部已修）**：

| # | 发现（含复现方式） | 处置 |
|---|---|---|
| 1 | `day_over_day` 整数溢出：adapter 对 token 数只验非负，构造 1e17 输入 → debug 构建 panic、release 显示 `-8446…%`（同工具链逐字节复现） | **已修**（ca344c7）：中间量 i128 |
| 2 | 事件循环内层排水无上限：resize 风暴/恶意 pty 持续供事件可无限饿死渲染、fetch 消息与 spinner | **已修**：单帧排水上限 64，超限落帧后下轮继续 |
| 3 | POSIX 删除重建库 → 连接读孤儿 inode 永远成功、版本恒等、`updated hh:mm` 还在走——版本轮询唯一看不见的陈旧（Windows 端因共享冲突不可构造，实测确认） | **已修**：Fetcher 逐轮比对 (dev, inode) 身份，换文件即重开重握手；unix 门控回归测试走 CI Linux 道 |
| 4 | 宽度 0 时截断仍发一格 `…`（外观级，ratatui 会裁） | **已修**：0 预算返回空串 |

对抗方确认存活的攻击面：`invalidate` 与并发 tick 的全部交错（强制刷新不丢失；TOCTOU 已由"以复合事务实际所见版本为准"关闭）、overview 事务在早退错误路径的回滚（对照 rusqlite 0.40.1 源码验证 Drop 语义）、读池在 panic/取消路径的连接归还、全部渲染函数在 0/1/2 尺寸与空数据下的防 panic、format 系列在 i64 极值的安全性。

修复后门禁重跑：**215 测试全过**（+3），clippy -D warnings 干净，deny 四项 ok。

**评审移交上游的一条**：adapter 侧对 token 计数只验非负、无上界（`tokly-adapter-claude`）——展示侧已加固，但摄取侧是否加合理性上界属 core/adapter 轨道，移交维护者。
